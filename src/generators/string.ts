...
export class StringGenerator extends BaseGenerator<string> {
  ...
  private generateUrl(fieldName: string): string {
    if (
      ...
      return `https://api.dicebear.com/7.x/avataaars/svg?seed=${faker.string.alphanumeric(
        8
      )}`;
    }
  ...
  private generateStringId(context: GenerationContext): string {
    ...
    // Check if default is uuid - must return valid UUID format for database compatibility
